...
test('unassigned identity can leave rejected SSO and sign in with another account', async ({
  ...
  const tokenResponse = await request.post(
    env.AUTH_URL + '/realms/rare-os/protocol/openid-connect/token',
    {
      form: {
  ...
  const created = await request.post(env.AUTH_URL + '/admin/realms/rare-os/users', {
    headers,
    data: {
      username: email,
      email,
  ...
  try {
    ...
    await request.delete(env.AUTH_URL + parsed.pathname, { headers });
  }
});
