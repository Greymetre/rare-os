...
## Restricted checkout permissions (EACCES migration repair)

The deploy process retains umask 077 for receipts/backups, but git checkout runs with 022. `prepare-source.mjs` makes only tracked files readable and their directories traversable, including the bind-mounted Keycloak theme; untracked .env/overrides/backups remain private. Docker normalizes packaged source read permissions after build, keeping the runtime non-root. A no-database runtime preflight reads migrations and entry points before services stop. Update the installed root-owned deploy command when deploying this repair. Existing images containing mode-600 migrations must be rebuilt; changing host files alone does not repair an already-built image.
